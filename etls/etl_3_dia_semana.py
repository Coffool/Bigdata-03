...
from etls.constants import RAW_INVOICE, RAW_INVOICE_LINE, PROC_DIA_SEMANA
...
df = df.withColumn("dia_semana", F.date_format("InvoiceDate", "EEEE"))

agg = df.groupBy("dia_semana").agg(
...
output_path = f"s3://{bucket}/{PROC_DIA_SEMANA}snapshot={timestamp}/"
agg.write.mode("overwrite").parquet(output_path)

print("✓ ETL 3 completado exitosamente")
